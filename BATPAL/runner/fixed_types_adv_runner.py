...
class FixedTypesAdvRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        self.n_adversaries = algo_args["algo"].get("n_adversaries", 1)
        ...
        self.use_belief = algo_args["algo"].get("use_belief", True)  # whether to use belief (ablation study)
        ...
        if self.share_param:
            ...
            self.external_adv_types = []
            self.adv_model_types_dir = algo_args["algo"].get("adv_model_types_dir", None)
            for i in range(self.n_severity_types):
                self.external_adv_types.append(PolicyAgent(80, 64, 11 , False, True)) #2s3z(80, 64, 11), MMM(160,128,16), spread(18,128,5), LBF(45,128,6)
            self.external_adv_hidden_size = 64
            for i, adv in enumerate(self.external_adv_types):
                adv.load_model(self.adv_model_types_dir[i])
            ...
            if self.adv_model_dir is not None:
                ...
                if self.evaluate_external:
                    self.external_adv = PolicyAgent(80, 64, 11 , False, True) #2s3z(80, 64, 11), MMM(160,128,16), spread(18,128,5), LBF(45,128,6)
    ...
    def run(self):
        ...
        for episode in range(1, episodes + 1):
            ...
            if self.random_adversary:                
                self.agent_adversary = np.random.choice(range(self.num_agents), size=self.n_adversaries, replace=False)
            ...
            for adv in self.agent_adversary:
                self.ground_truth_type[self.episode_adversary, :, adv] = 1
            ...
            for step in range(self.episode_length):
                ...
                for adv in self.agent_adversary:
                    input_actions[self.episode_adversary, adv] = adv_actions[self.episode_adversary, adv]
                ...
                share_obs = self._pad_obs(share_obs, 2 * self.belief_shape)  # we add both the belief and the true type
    ...
    def warmup(self):
        ...
        self.external_adv_hidden = torch.zeros((self.n_severity_types, self.n_rollout_threads, self.external_adv_hidden_size), dtype=torch.float32)           
    ...
    def collect_adv(self, step):
        ...
        for agent_id in range(self.num_agents):
            ...
            if not self.use_belief:
                belief = np.zeros((self.n_rollout_threads, self.belief_shape))    
        ...
        external_obs = self.actor_buffer[self.agent_adversary[0]].obs[step].copy()
        external_adv_obs = torch.tensor(external_obs[:, :-self.belief_shape -self.num_agents], dtype=torch.float32) #-self.num_agents for SMAC
    
        external_adv_act, self.external_adv_hidden[self.severity_ind] = self.external_adv_types[self.severity_ind].compute_action(external_adv_obs, 
                                                self.external_adv_hidden[self.severity_ind], self.actor_buffer[self.agent_adversary[0]].available_actions[step].copy())        
        ...
        for agent_id in range(self.num_agents):
            ...
            adv_action = external_adv_act.copy()            
            adv_action_collector.append(_t2n(adv_action))
            adv_action_log_prob_collector.append(_t2n(adv_action_log_prob))
            adv_rnn_state_collector.append(_t2n(adv_rnn_state))
        # [self.envs, agents, dim]
        ...
        if self.state_type == "EP":
            ...
            if self.teacher_forcing and np.random.rand() < self.true_type_prob:
                belief_central = self.ground_truth_type
            if not self.use_belief:
                belief_central = np.zeros_like(belief_central)    
    ...
    def insert(self, data):
        ...
        self.external_adv_hidden[:, dones_env == True] = torch.zeros(((dones_env == True).sum(
        ...
        adv_mask = np.isin(np.arange(self.num_agents), self.agent_adversary)
        adv_active_masks[:, ~adv_mask] = 0
    ...
    def baseline_insert(self, data):
        ...
        #self.baseline_values_episode += self.baseline_discounts* rewards[:, 0, 0]
        #self.baseline_values_list = np.concatenate((self.baseline_values_list, self.baseline_values_episode[dones_env == True]))
        #self.baseline_discounts *= self.gamma
        #self.baseline_discounts[dones_env == True] = 1.0
        #self.baseline_values_episode[dones_env == True] = 0.0
        rnn_states[dones_env == True] = np.zeros(((dones_env == True).sum(
        ), self.num_agents, self.recurrent_N, self.rnn_hidden_size), dtype=np.float32)
        adv_rnn_states[dones_env == True] = np.zeros(((dones_env == True).sum(
        ), self.num_agents, self.recurrent_N, self.rnn_hidden_size), dtype=np.float32)
    ...
    def eval(self):
        ...
        while True:
            ...
            for agent_id in range(self.num_agents):
                ...
                if not self.use_belief:
                    eval_belief = np.zeros((self.n_eval_rollout_threads, self.belief_shape))
    ...
    def _eval_adv(self, severity_ind):
        if self.random_adversary:
            adv_id = np.zeros((self.n_eval_rollout_threads,self.n_adversaries), dtype=int)
            for i in range(self.n_eval_rollout_threads):
                adv_id[i] = np.random.choice(self.num_agents, self.n_adversaries, replace=False)
        else:
        ...
        eval_external_adv_hidden = np.zeros((self.n_severity_types, eval_obs.shape[0],
                                            self.external_adv_hidden_size), dtype=np.float32) 

        while True:

            ground_truth_type = np.zeros((self.n_eval_rollout_threads, self.num_agents, self.belief_shape))
            # Set ground_truth_type for multiple adversaries per thread
            for thread in range(self.n_eval_rollout_threads):
                for adv in adv_id[thread]:
                    ground_truth_type[thread, :, adv] = 1
            ground_truth_type[:, :, -self.n_severity_types + severity_ind] = 1

            external_adv_obs = torch.zeros((eval_obs.shape[0], eval_obs.shape[2] - self.belief_shape -self.num_agents), dtype=torch.float32) #-self.num_agents for SMAC
            for thread in range(self.n_eval_rollout_threads):
                external_adv_obs[thread] = torch.tensor(eval_obs[thread, adv_id[thread], :-self.belief_shape -self.num_agents], dtype=torch.float32) #-self.num_agents for SMAC
            external_adv_act, eval_external_adv_hidden[severity_ind] = self.external_adv_types[severity_ind].compute_action(external_adv_obs, eval_external_adv_hidden[severity_ind],
                                                                             eval_available_actions[np.arange(self.n_eval_rollout_threads)[:, None], adv_id].reshape(self.n_eval_rollout_threads, -1)) 
            ...
            for thread in range(self.n_eval_rollout_threads):
                for adv in adv_id[thread]:
                    eval_actions[thread, adv] = external_adv_act[thread]
            ...
            eval_external_adv_hidden[:, eval_dones_env == True] = np.zeros(((eval_dones_env == True).sum(
            ), self.external_adv_hidden_size), dtype=np.float32)
            eval_masks = np.ones(
                (eval_obs.shape[0], self.num_agents, 1), dtype=np.float32)
            eval_masks[eval_dones_env == True] = np.zeros(
